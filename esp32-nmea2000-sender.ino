...
// #include <memory>
#include <N2kMessages.h>        // also https://github.com/ttlappalainen/NMEA2000
...
void setup() {
  ...
  //timer = timerBegin(80);                                                // this returns a pointer to the hw_timer_t global variable
  // // 0 = first timer
 ...
 for (i = 0; i < 6; i++) id += (chipid[i] << (7 * i));
  ...
  NMEA2000.SetProductInformation("1", // Manufacturer's Model serial code
                                 ...
                                 "ESP NMEA2000 Sender",  // Manufacturer's Model ID
...
void loop() {
  ...
  BatteryVolt = ((BatteryVolt * 15) + (1000 * ADC_Calibration_Value2 / 4096)) / 16; // This implements a low pass filter to eliminate spike for ADC readings
  ...
  FuelLevel = ((FuelLevel * 15) + (1024 * ADC_Calibration_Value1 / 4096)) / 16; // This implements a low pass filter to eliminate spike for ADC readings
  ...
  EngineRPM = ((EngineRPM * 5) + 1000 * RPM_Calibration_Value) / 6 ; // This implements a low pass filter to eliminate spike for RPM measurements
